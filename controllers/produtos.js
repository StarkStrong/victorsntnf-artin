...
route.put("/produtos", async (req, res) => {
    ...
    if (dados == null) {
        res.send("O Produto não foi encontrado")
        return false
    }
        var mudanca = 'Pronto, Produto alterado com sucesso \n';
    if(preco != dados.preco){
        mudanca += 'Preço foi alterado com sucesso (De "'+dados.preco+'" para "'+preco+' ")\n';
    }
    if(nome != dados.nome){
        mudanca += 'Nome foi alterado sucesso (De "'+dados.nome+'" para "'+nome+' ")\n';
    }
    if(imagem != dados.imagem){
        mudanca += 'Imagem foi alterada com sucesso (De "'+dados.imagem+'" para "'+imagem+' ")\n';
    }
    if(descricao != dados.descricao){
        mudanca += 'Descrição alterada com sucesso (De "'+dados.descricao+'" para "'+descricao+' ")\n';
    }
    if(quantidade != dados.quantidade){
        mudanca += 'Quantidade alterada com sucesso (De "'+dados.quantidade+'" para "'+quantidade+' ")\n';
    }
